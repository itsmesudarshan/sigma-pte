...
class Question(Base):
    ...
    q_type = Column(String, index=True)           # rw_fill_blanks, fill_blanks, reorder, mcq_single, mcq_multi
    difficulty = Column(String, index=True)        # easy, medium, hard
    tags = Column(JSON, default=list)
    title = Column(String)
    passage = Column(Text)                         # main text/passage
    content = Column(JSON)                          # type-specific structured content (options, blanks, paragraphs...)
    correct_answer = Column(JSON)                   # type-specific correct answer key
