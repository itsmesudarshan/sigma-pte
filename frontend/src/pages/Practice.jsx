...
export default function Practice() {
  ...
  const canSubmit = isWriting ? (userAnswer.text || '').trim().length > 0 : Object.keys(userAnswer).length > 0;
      ...
      {!result ? (
        ...
        <div style={{ marginTop: 20, display: 'flex', gap: 20, alignItems: 'center', padding: 20, background: 'var(--paper-raised)', border: '1px solid var(--line)', borderRadius: 'var(--radius-lg)' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <button
              onClick={() => navigate(isWriting ? `/question-bank?module=writing&q_type=${question.q_type}` : `/question-bank?q_type=${question.q_type}`)}
